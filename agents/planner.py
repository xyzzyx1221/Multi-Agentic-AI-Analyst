...
def planner_node(state: AgentState):
    ...
    # Use selected_schema if available, otherwise fallback to full db_schema
    schema_to_use = state.get("selected_schema") or state.get("db_schema", {})
    
    prompt = f"Schema: {json.dumps(schema_to_use)}\n\nQuery: {contextual_query}"
